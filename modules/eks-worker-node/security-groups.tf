resource "aws_security_group" "eks_learning_workers_sg" {
...
resource "aws_security_group_rule" "worker_to_worker_tcp" {
  ...
  from_port                = 0
  protocol                 = "tcp"
  security_group_id        = aws_security_group.eks_learning_workers_sg.id
  source_security_group_id = aws_security_group.eks_learning_workers_sg.id
  to_port                  = 65535
  type                     = "ingress"
}
